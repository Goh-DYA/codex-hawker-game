...
The current source also includes per-item visual recipes, visible meals and lifecycle animation, balanced stall selection, globally unique obstacle-safe configurable queues, geometry-aware queue-flow guidance, coherent expansion-boundary migration, and functional signage/facility effects. The final current-source gate passes: typecheck, lint, 11 Vitest files / 61 tests, production build, and rendered-output/offline-shell assertions. The replacement `dist/` artifact and service-worker build `3e25823ed59c` are recorded in the test and performance reports.
...
| Automated QA | Current-source release gate passed | Final release checks passed typecheck, lint, 11 files / 61 tests, production build, and 2 rendered-output/offline-shell tests |
